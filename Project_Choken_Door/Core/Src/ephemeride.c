...
extern int8_t UTC;
extern int8_t latitude;            // définis dans main.c
extern int16_t longitude;                   // définis dans main.c
...
void calculerEphemeride()
{
  ...
  afficherHeure(coucher);
    ...
    // === DÉBUT DE TON AJOUT (Maintenant BIEN À L'INTÉRIEUR de la fonction) ===
    minGPSF = minGPSF + minretard + 30; // ajout du retard réglable + fixe 30 min
    if (minGPSF > 119)
    {
        minGPSF = minGPSF - 120;
        heureGPSF = heureGPSF + 2;
    }
    else if (minGPSF > 59)
    {
        minGPSF = minGPSF - 60;
        heureGPSF = heureGPSF + 1;
    }

    heureGPSO += UTC;               // en france 2h de décalage UTC en été
    heureGPSF += UTC;               // en france 1h de décalage UTC en hiver

    if (heureGPSO > 24)
    {
        heureGPSO = heureGPSO - 24;
    }
    if (heureGPSO < 0)
    {
        heureGPSO = heureGPSO + 24;
