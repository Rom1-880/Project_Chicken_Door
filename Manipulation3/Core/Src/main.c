  ...
  while (1)
    {
	  ...
	  adc_bat_value = (uint16_t)adc_buffer[0];   // IN4 (PA4) — pont diviseur batterie | La batterie est sur le Rank 2 -> adc_buffer[1]
...
// Tension Réel des piles (Application du Coef)
 ...
 /*   	  bat_pourcentage = (int)(((v_bat_lisse - V_MIN)/(V_MAX - V_MIN))*100.0f);
   */
    	  bat_pourcentage =(-31.4f *v_bat_reel * v_bat_reel * v_bat_reel)+(483.0f * v_bat_reel * v_bat_reel) -(2389.0f * v_bat_reel) + 3842.0f;
      } else {
    	  bat_pourcentage = 0;
      }
      ...
      //Décomposition pour affichage sans %f
        ...
        //==================================================================//
          ...
          int len2 = sprintf(msg, "Tension: %d.%02dV | Energie: %d%%\r\n\r\n",
                  v_entier, v_dec, bat_pourcentage); // ancienne partie de code
